...
create table property (
  ...
  bq_dataset text,                          -- e.g. 'tnabushire', 'buscharter' — per-property dataset in BQ
...
**Decided 2026-05-06:** Adam's pipeline package writes only to BQ. A dedicated BQ dataset (`data-hub-468216.seo_platform_ops`) holds the operational state. A sync layer materializes that dataset into Supabase. The app reads/writes Supabase.

BQ datasets in play:
- `phil_lasry`, `tnabushire`, `buscharter`, etc. — existing per-client raw datasets (crawl results, GSC pulls, keyword research dumps). Untouched by this spec.
- `seo_platform_ops` (new) — operational state mirror: pages with audit decisions, keywords with status, brand DNA, project brain, signals, playbook runs, briefs. Adam's package owns writes here.

Sync directions:

| Direction | What | How | Frequency |
|---|---|---|---|
| BQ ops → Supabase | All operational state | Scheduled ETL (Cloud Run job or pg_cron pulling via Supabase FDW) reads `seo_platform_ops` and upserts into Supabase | On pipeline run + hourly catch-up |
| Supabase → BQ ops | Human/agent edits in the app (audit decisions, brain entries, brief status changes) | Nightly `pg_cron` job pushes deltas back to `seo_platform_ops` | Daily |
| BQ raw → App | Read-only analytics views over per-client raw datasets | App's BQ client reads directly | On demand |

Conflict resolution: Supabase wins on any field that the app/humans can edit (audit decisions, brain entries, brief status). BQ wins on fields only the pipeline writes (crawl metadata, embeddings, search volumes). Schema-level field ownership prevents most conflicts.
